...
#' @keywords internal
#' 
#' Make wrapper function that formats the data as a list and blah blah
infer_format_ifti <- function(BOLD, verbose=FALSE){
  ...
  # The xifti, gifti, or niftiImage data object
    ...
    Bformat <- "gifti"
  } else if (inherits(BOLD, "niftiImage") | inherits(BOLD, "niftiExtension")) {
    Bformat <- "nifti"
  ...
  } else if (inherits(BOLD[[1]], "niftiImage") | inherits(BOLD[[1]], "niftiExtension")) {
    if (all(vapply(BOLD, inherits, what="niftiImage", FALSE) | vapply(BOLD, inherits, what="niftiExtension", FALSE))) {
      Bformat <- "nifti_list"
    } else {
